...
- Interpret `2x4`, `2列×4段`, or requests to add four sections below the existing two-section row as exactly two sections across and four section rows down (eight total). Do not substitute four sections across and two rows. Completion requires a fresh normal-view screenshot where all four stacked two-section rows are visibly countable; JSON zone counts or layout tests alone are insufficient.
